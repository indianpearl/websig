// --- Mapa e camadas (mantido) ---
    ...
    const vehicles = []; // {id, code, plate, brand, model, capacity, state, marker, assignedEcopoints:[]}
    const vehicleMarkersLayer = L.layerGroup().addTo(map);

    function randomPlate(){ const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'; const nums = () => Math.floor(1000 + Math.random()*9000); return letters.charAt(Math.floor(Math.random()*26)) + letters.charAt(Math.floor(Math.random()*26)) + '-' + nums(); }
    function randomBrandModel(){ const brands = [['Volvo','FMX'],['Mercedes','Arocs'],['Scania','P-series'],['Iveco','Stralis'],['MAN','TGS'],['Renault','C-Series']]; return brands[Math.floor(Math.random()*brands.length)]; }
    function randomCapacity(){ return [6,8,10,12,14,16][Math.floor(Math.random()*6)]; } // m3
    ...
    function createVehicles() {
      ...
      for (let i=0;i<VEHICLE_COUNT;i++){
        ...
        const state = stateRand < 0.75 ? 'operacional' : (stateRand < 0.9 ? 'manutenção' : 'inoperacional');
        const v = {
    ...
    function renderVehicleList(){
      const op = vehicles.filter(v=>v.state==='operacional').length;
      activeVehiclesEl.textContent = op;
      log(`Criadas ${vehicles.length} viaturas (${op} operacionais).`);
    }

    function truckIcon(color='#2b7') {
      const svg = encodeURIComponent(`<svg xmlns='const iconViatura = L.icon({
  iconUrl:'https://cdn-icons-png.flaticon.com/512/1995/1995470.png',
  iconSize:[35,35]
});' width='36' height='24' viewBox='0 0 36 24'><rect x='1' y='6' width='22' height='12' rx='2' fill='${color}' stroke='#fff' stroke-width='1'/><rect x='23' y='10' width='10' height='8' rx='1' fill='${color}' stroke='#fff' stroke-width='1'/><circle cx='9' cy='20' r='2' fill='#333'/><circle cx='27' cy='20' r='2' fill='#333'/></svg>`);
...
async function loadEcopontosFromServer(options = {}) {
  const serverPath = options.serverPath || 'websig/ecopontos.geojson.json';
